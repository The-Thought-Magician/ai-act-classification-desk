    ...
    <main className="min-h-screen bg-slate-950 text-white">
      ...
      <section className="mx-auto max-w-5xl px-6 py-20 text-center">
        ...
        <div className="mt-14 grid gap-6 md:grid-cols-2">
          {/* Free */}
          <div className="rounded-2xl border border-indigo-500/40 bg-slate-900/60 p-8 text-left ring-1 ring-indigo-500/20">
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-bold">Free</h2>
              <span className="rounded-full border border-emerald-500/30 bg-emerald-500/15 px-3 py-1 text-xs font-medium text-emerald-300">Current plan</span>
            </div>
            <div className="mt-4 text-4xl font-black">€0<span className="text-base font-medium text-slate-500">/month</span></div>
            <p className="mt-2 text-sm text-slate-400">Full access to the entire compliance workbench.</p>
            ...
            <Link href="/auth/sign-up" className="mt-8 block rounded-lg bg-indigo-600 px-6 py-3 text-center font-semibold text-white hover:bg-indigo-500">
              Get started free
            </Link>
          </div>
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-8 text-left">
            ...
            <div className="mt-4 text-4xl font-black">€49<span className="text-base font-medium text-slate-500">/month</span></div>
            <p className="mt-2 text-sm text-slate-400">Everything in Free, plus a billing relationship and priority support. No additional feature gates.</p>
            <ul className="mt-6 space-y-2 text-sm text-slate-300">
            ...
            <button
              ...
              className="mt-8 block w-full rounded-lg border border-slate-700 bg-slate-800 px-6 py-3 text-center font-semibold text-slate-200 hover:bg-slate-700 disabled:opacity-50"
            >
              {busy ? 'Starting checkout...' : stripeEnabled ? 'Upgrade to Pro' : 'Billing not configured'}
            </button>
            {!stripeEnabled && (
              <p className="mt-3 text-center text-xs text-slate-500">Stripe is not configured, so checkout is disabled. All features remain free.</p>
            )}
        ...
        {planName && (
          <p className="mt-8 text-sm text-slate-500">You are currently on the <span className="font-medium text-slate-300">{planName}</span> plan.</p>
        )}
      </section>
